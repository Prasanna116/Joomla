...
class CoalawebSocialLinksHelper {   
    ...
    function getTwitterBookmark($title, $link, $size, $linknofollow, $via, $linkText) {
        $linkTextOn = ($linkText ? JText::sprintf("MOD_COALAWEBSOCIALLINKS_BOOKMARK", "Twitter") : '');
        $output[] = '<li>';
        $output[] = '<a class="twitter' . $size . '" href="https://twitter.com/intent/tweet?text=' . $title . '&amp;url=' . $link . '' . $via .'" title="' . JText::sprintf("MOD_COALAWEBSOCIALLINKS_BOOKMARK", "Twitter") . '" ' . $linknofollow . 'target="_blank">' . $linkTextOn . '</a>';
        $output[] = '</li>';
        return implode("\n", $output);
    }
