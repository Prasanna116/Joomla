...
if (COM_CWSOCIALLINKS_PRO == 1) {
    // Check Content Social Likes plugin
    if (JPluginHelper::isEnabled('content', 'cwsociallikes', true) == false) {
        echo JText::_('COM_CWSOCIALLINKS_NOCONTENTPLUGIN_GENERAL_MESSAGE');
    }

    // Check Zoo Elements plugin
    if (JPluginHelper::isEnabled('system', 'cwzooelements', true) == false) {
        echo JText::_('COM_CWSOCIALLINKS_NOZOOPLUGIN_GENERAL_MESSAGE');
    }

    // We can't use our update plugin unless the Joomla installation is at least 2.5.19
    if (version_compare(JVERSION, '2.5.19', '<')) {
        echo JText::_('COM_CWSOCIALLINKS_UPDATE_MINVERSION_MSG');
    } else if ((version_compare(JVERSION, '3.0', '>')) && (version_compare(JVERSION, '3.2.1', '<'))) {
        echo JText::_('COM_CWSOCIALLINKS_UPDATE_MINVERSION_MSG');
    }
}
